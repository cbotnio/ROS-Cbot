...
def gpsCallback(data):
	global sendData
	sendData["Latitude"] = round(data.latitude,8)
	sendData["Longitude"] = round(data.longitude,8)
...
def parseData(message):
	global control_client, guidance_client
	...
	if(int(message["AUV_mode"])):
		...
		if(int(message["MissionInactive"])):
			...
			guidance_client.update_configuration({"thrusters_on":0})
			...
			guidance_client.update_configuration({"thrusters_on":1})
		...
		guidance_client.update_configuration({"thrusters_on":int(message["THRUSTERS_ON"])})
...
guidance_client = dynamic_reconfigure.client.Client("actuators_node", timeout=0, config_callback=dynamicCallback)
